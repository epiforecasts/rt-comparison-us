...
colours <- c(
  ...
  "Admissions" = "#e7298a",
  "All deaths" = "#1b9e77",
  "Hospital deaths" = "#66a61e"
  )

# Tests & admissions: #AE4D9F darker & #E180D2 lighter
# Tests & all deaths: #488795 & #7BBAC8
# Tests & hospital deaths: #6E8B69
# Admissions & all deaths: #816481
# Admissions & hospital deaths: #A76854
...
plot_national_test_adm <- ratio_national %>%
  ggplot(aes(x = date)) +
  ...
  theme(strip.text.x = element_blank()) +
  theme(axis.text.x = element_blank()) +
  labs(title = "Rt(public tests) / Rt(hospital admissions)", 
       y = "Ratio", x = "", col = "Data", fill = "Data")

#  # Public tests / all deaths
plot_national_test_deaths <- ratio_national %>%
  ggplot(aes(x = date)) +
...
plot_national_test_hospdeaths <- ratio_national %>%
  ggplot(aes(x = date)) +
...
plot_national_adm_deaths <- ratio_national %>%
  ggplot(aes(x = date)) +
...
# plot_national_adm_hospdeaths <- ratio_national %>%
#   ggplot(aes(x = date)) +
...
#   theme(panel.spacing.x = unit(0.5, "cm")) +
#   theme(strip.text.x = element_blank()) +
#   labs(title = "Rt(hospital admissions) / Rt(hospital deaths)", 
#        y = "Ratio", x = "", 
#        col = "Data", fill = "Data")
...
plot_all_national <- 
  plot_count_national +
  plot_rt_national +
  plot_national_test_adm +
  plot_national_test_deaths +
  plot_national_test_hospdeaths +
  plot_national_adm_deaths +
  # plot_national_adm_hospdeaths +
  plot_layout(nrow = 6) +
  theme(legend.position = 'top')

ggsave(here::here("figures", "national_rt_and_ratios.png"),
       plot_all_national, dpi = 330, height = 9, width = 8)
