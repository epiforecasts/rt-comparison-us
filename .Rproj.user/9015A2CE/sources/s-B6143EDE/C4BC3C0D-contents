...
national <- list(
  ...
  cases_public = report_rt("nowcast/public-tests/national", creation_date = target)[, type := "Public tests"],
  deaths_hospital = report_rt("nowcast/hospital-deaths/national", creation_date = target)[, type := "Hospital deaths"]
  )

national <- data.table::rbindlist(national)
national <- national[!Geography %in% "United Kingdom"]
...
# 
# combined <- data.table::copy(overall)[!type %in% "Lab reports"][, estimates := .N, 
#                                                                         by = .(`Value Day`, `Value Month`, 
#                                                                                `Value Year`, Geography)][,
...
#                                             `Creation Year`, `Value Day`, `Value Month`, 
#                                             `Value Year`, Geography, ValueType,
#                                             Model),
...
all_r <- all_r[, date := lubridate::dmy(paste0(`Value Day`, "-", `Value Month`, "-", `Value Year`))][,
                 type := factor(type, levels = c("Combined", "Public tests", "Admissions", "All deaths", "Hospital deaths"))]
...
last_estimate <- data.table::copy(all_r)[, estimates := .N, by = .(`Value Day`,
                                                                   `Value Month`,
                                                                   `Value Year`, 
                                                                   Geography)]
